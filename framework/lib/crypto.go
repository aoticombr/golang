...
var (
	Key   = string("YUQL23KL23DF90WI5E1JAS467NMCXXL6JAOAUWWMCL0AOMM4A4VZYW9KHJUI2347EJHJKDF3424S  klk3lakdjsl9rtikj ")
	Range = 256
)
...
func Encrypt(src string) string {
	...
	offset = rand.Intn(Range)
	ret = fmt.Sprintf("%1.2x", offset)
	...
	for srcpos = 0; srcpos < len(src); srcpos++ {
		...
		srcasc = srcasc ^ int(Key[keypos-1])

		ret += fmt.Sprintf("%1.2x", srcasc)
...
func Decrypt(src string) string {
	...
	offset = intFromHex(src[0:2])
	srcpos = 2

	for srcpos < len(src) {
		srcasc = intFromHex(src[srcpos : srcpos+2])
		...
		tmpsrcasc = srcasc ^ int(Key[keypos-1])
